...
#####HELPER FUNCTIONS
    ...
    def getPuzzle(self, difficulty): #difficulty is variable set between 1 to 4
        html_doc = requests.get("https://nine.websudoku.com/?level={}".format(difficulty)).content #will fetch the sudoku puzzle from the link. #diffuclty will be the variable and be fetch
        soup = BeautifulSoup(html_doc, 'html.parser')   
        #print(soup.prettify())
        ids = ['f00', 'f01', 'f02', 'f03', 'f04', 'f05', 'f06', 'f07', 'f08', 'f09', 'f10', 'f11',
        'f12', 'f13', 'f14', 'f15', 'f16', 'f17', 'f18', 'f19', 'f20', 'f11', 'f22', 'f23',
    ...
    def drawNumbers(self, window):
        for yidx, row in enumerate(self.grid):
            for xinx, num in enumerate(row):
                if num != 0:
                    pos = [(xinx*cellSize)+gridPos[0], (yidx*cellSize)+gridPos[1]]
...
#
 #           else:
    ...
    def loadButtons(self):
        ...
        self.playingButtons.append(Button(140, 40, WIDTH//7, 40, function=self.getPuzzle, color=(117,172,112), params="1", text="Easy"))
        self.playingButtons.append(Button(WIDTH//2-(WIDTH//7)//2, 40, WIDTH//7, 40, function=self.getPuzzle, color=(204,197,110), params="2", text="Medium"))
        self.playingButtons.append(Button(380, 40, WIDTH//7, 40, function=self.getPuzzle, color=(199,129,48), params="3", text="Hard"))
        self.playingButtons.append(Button(500, 40, WIDTH//7, 40, function=self.getPuzzle, color=(207,68,68), params="4", text="Evil"))
    ...
    def load(self):
        ...
        #locking cells from original board
        for yidx, row in enumerate(self.grid):
